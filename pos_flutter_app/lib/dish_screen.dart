...
class _DishSearchScreenState extends State<DishSearchScreen> {
  ...
  void goToCart() async {
    final updatedCart = await Navigator.push(
        ...
        builder: (_) => CartScreen(
            ...
            selectedTable: widget.selectedTable,  // <-- pass selectedTable here
        ),
        ),
    );
  ...
  Future<bool> _onWillPop() async {
    Navigator.pop(context, cart); // Only return the cart, as expected
    return false;
  }
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: Text(
            'Table: ${widget.selectedTable['label'] ?? widget.selectedTable['number'] ?? 'Unknown'}',
          ),
          actions: [
            IconButton(
